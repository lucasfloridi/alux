...
//PERSONALIZAÇÕES ESTÃO IDENTIFICADAS COMO FSW TOTVS IP
...
if lSuframa .and. Len(aICMSZFM)>0 
	If!(lMvNFLeiZF)
		if aIcmsZFM[1] > 0 .and. empty(aProd[15])	
			...
			If aProd[33] > 0
				//cMensDeson := 'Valor do ICMS abatido: R$ '+ ConvType(aIcmsZFM[1]-aProd[31]-aProd[32],15,2)+ ' ( '+AllTrim(Str(aProd[33]))+'% sobre R$ ' +ConvType(aProd[10] + aProd[13] + aProd[14],15,2)+ ' ).' - FSW TOTVS IP
			Else
				//cMensDeson := 'Valor do ICMS abatido: R$ '+ ConvType(aIcmsZFM[1]-aProd[31]-aProd[32],15,2)+ ' ( '+'7% sobre R$ ' + ConvType(aProd[10]- IIF(cTipo == '1', aProd[31]+aProd[32], 0),15,2)+ ' ).' - FSW TOTVS IP
			EndIf	
...
//INICIO TRECHO COMENTADO PERSONALIZADO - FSW TOTVS IP
/*
...
EndIf
*/
//FIM TRECHO COMENTADO PERSONALIZADO - FSW TOTVS IP
